...
function like() {
  console.log('ok');
  const button = document.querySelector('.place-card__buttons-like');
...
function removeCard () {
    let card = document.querySelector('.place-card');
    card.remove();
  }
  ...
  function createCardfromArr (el) {
    let card = userTemplate.querySelector('.place-card').cloneNode(true);
    card.querySelector('.place-card__image').src = el.link;
    card.querySelector('.place-card__subtitle').textContent = el.name;
    return card;
  }
  
  function saveCardfromArr () {
    createCardfromArr();
    addCard();
  }
